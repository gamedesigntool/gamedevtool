...
function DocEditor({value,color,onChange,insertRef}:{value: string; color: string; onChange: (value: string) => void; insertRef: InsertHtmlRef}){
  ...
  const syncFromDom=useCallback((options?: {onlyIfChanged?: boolean})=>{
  ...
  return(
    <div style={{display:'flex',flexDirection:'column',height:'100%',overflow:'hidden',position:'relative'}}>
      ...
      <div ref={edRef} contentEditable suppressContentEditableWarning onClick={handleClick} onInput={()=>syncFromDom()} onBlur={()=>syncFromDom({onlyIfChanged:true})} data-placeholder="Comece a escrever ou use a IA para gerar conteúdo..." style={{flex:1,overflowY:'auto',padding:'20px 24px',outline:'none',lineHeight:1.8,fontSize:14,color:'var(--gdd-text)',caretColor:color}}/>
...
function GDDHubInner(){
  ...
  const projectFormFields: {key: keyof typeof form; label: string}[]=[{key:'name',label:'Nome do jogo *'},{key:'genre',label:'Gênero'},{key:'platform',label:'Plataforma'}];
  ...
  if(view==='dashboard')return(
    <div style={S.app}>
      <div style={{padding:'0 24px',height:54,borderBottom:'1px solid '+th.border2,display:'flex',alignItems:'center',justifyContent:'space-between',background:th.bg,position:'sticky',top:0,zIndex:20}}>
        ...
        <div style={{display:'flex',gap:8,alignItems:'center'}}><LangToggle lang={lang} setLang={setLangControl}/><ThemeToggle theme={theme} setTheme={setThemeControl}/><AuthControls authSession={authSession} setAuthSession={setAuthSession}/><button style={S.btn()} onClick={()=>setShowNew(true)}>{t.dash_new}</button></div>
      </div>
